...
evalAtom (Par c1 c2) s =
  case (nextAtom c1, nextAtom c2) of
    ...
    otherwise -> runDistT $ runExceptT $ runStateT (small (Par c1 c2)) s
  ...
  case (nextAtom c1) of
    ...
    True -> if (nextAtom c2 == False)
            then let listDist = evalAtom c1 s
                 in [compParR dist c2 | dist <- listDist]
            else runDistT $ runExceptT $ runStateT (small (Par c1 c2)) s
evalAtom c s = runDistT $ runExceptT $ runStateT (small c) s                 
...
nextAtom (Par c1 c2) = nextAtom c1 || nextAtom c2
