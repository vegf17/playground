...
--Functions for user defined gates--
...
applyUserGate g nums s = let m = pickUserGate g
                             n_rows = nrows m
                         in case n_rows of
                              2 -> apply1QbitGate m nums s
                              4 -> apply2QbitGate m nums s
                              otherwise -> applyNQbitGate m nums s

apply1QbitGate :: Op -> [Int] -> StQ -> StQ
apply1QbitGate m nums s = mult matrix s
  where matrix = applyToSomeQ m nums (numQubits s)
  ...
  | otherwise = if (q1 == q2)
                then error "apply2QbitGate (QuantumCal): The qubits in apply2QbitGate should be different"
                     ...
                     where q1 = head nums
                           ...
                           (m_tl, m_tr, m_bl, m_br) = splitBlocks 2 2 g
                           matrix00 = tensorProduct $ replaceByGate m0 [q1] (replaceByGate m_tl [q2] listId)
                           matrix01 = tensorProduct $ replaceByGate m01 [q1] (replaceByGate m_tr [q2] listId)
                           matrix10 = tensorProduct $ replaceByGate m10 [q1] (replaceByGate m_bl [q2] listId)
                           matrix11 = tensorProduct $ replaceByGate m1 [q1] (replaceByGate m_br [q2] listId)
  ...
  | otherwise = if ((length $ nub nums) /= (length nums))
                then error "applyNQbitGate (QuantumCalc): The qubits in applyNQbitGate should be all different"
                     ...
                     where q1 = head nums
                           ...
                           listId = gateList ident nqubits
                           i_div = (nrows g) `div` 2
                           (m_tl, m_tr, m_bl, m_br) = splitBlocks i_div i_div  g
                           m_A = aux_funcN m_tl q_rest nqubits listId
                           m_B = aux_funcN m_tr q_rest nqubits listId
                           m_C = aux_funcN m_bl q_rest nqubits listId
                           m_D = aux_funcN m_br q_rest nqubits listId
                           matrix00 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m0 [q1]) m) m_A
                           matrix01 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m01 [q1]) m) m_B
                           matrix10 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m10 [q1]) m) m_C
                           matrix11 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m1 [q1]) m) m_D
                           matrix = sumMatrices (sumMatrices matrix00 matrix01) (sumMatrices matrix10 matrix11)

aux_funcN :: Op -> [Int] -> Int -> [Op] -> [[Op]]
aux_funcN g nums nqubits listOp = case (length nums) of
  2 -> [matrix00, matrix01, matrix10, matrix11]
    where q1 = head nums
          q2 = last nums
          (m_tl, m_tr, m_bl, m_br) = splitBlocks 2 2 g
          matrix00 = replaceByGate m0 [q1] (replaceByGate m_tl [q2] listOp)
          matrix01 = replaceByGate m01 [q1] (replaceByGate m_tr [q2] listOp)
          matrix10 = replaceByGate m10 [q1] (replaceByGate m_bl [q2] listOp)
          matrix11 = replaceByGate m1 [q1] (replaceByGate m_br [q2] listOp)
  otherwise -> m_00 ++ m_01 ++ m_10 ++ m_11
    where q_head = head nums
          q_tail = tail nums
          i_div = (nrows g) `div` 2
          (m_tl, m_tr, m_bl, m_br) = splitBlocks i_div i_div  g
          m_00 = map (\m -> replaceByGate m0 [q_head] m) (aux_funcN m_tl q_tail nqubits listOp)
          m_01 = map (\m -> replaceByGate m01 [q_head] m) (aux_funcN m_tr q_tail nqubits listOp)
          m_10 = map (\m -> replaceByGate m10 [q_head] m) (aux_funcN m_bl q_tail nqubits listOp)
          m_11 = map (\m -> replaceByGate m1 [q_head] m) (aux_funcN m_br q_tail nqubits listOp)

sumListMat :: [Op] -> Op
sumListMat l = case length l of
  0 -> 0
  1 -> head l
  otherwise -> sumMatrices (head l) (sumListMat $ tail l)
--Functions for user defined gates--
...
-- Functions for user defined gates
applyUserGateOpDen :: String -> [Int] -> StQ -> StQ
applyUserGateOpDen g nums s = let m = pickUserGate g
                                  n_rows = nrows m
                              in case n_rows of
                                   2 -> apply1QbitGateOpDen m nums s
                                   4 -> apply2QbitGateOpDen m nums s
                                   otherwise -> applyNQbitGateOpDen m nums s

apply1QbitGateOpDen :: Op -> [Int] -> StQ -> StQ
apply1QbitGateOpDen m nums s = zeroIfSmallS $ mult (mult matrix s) (dagger matrix)
  where matrix = applyToSomeQ m nums (div (numQubits s) 2)
  

apply2QbitGateOpDen :: Op -> [Int] -> StQ -> StQ
apply2QbitGateOpDen g nums s
  | (length nums /= 2) =  error "First argument of function apply2QbitGateOpDen must be a list with two elements."
  | otherwise = if (q1 == q2)
                then error "The qubits in apply2QbitGateOpDen should be different"
                else zeroIfSmallS $ mult (mult matrix s) (dagger matrix)
                     where q1 = head nums
                           q2 = last nums
                           nqubits = (numQubits s) `div` 2
                           listId = gateList ident nqubits
                           (m_tl, m_tr, m_bl, m_br) = splitBlocks 2 2 g
                           matrix00 = tensorProduct $ replaceByGate m0 [q1] (replaceByGate m_tl [q2] listId)
                           matrix01 = tensorProduct $ replaceByGate m01 [q1] (replaceByGate m_tr [q2] listId)
...
--for (3+)-qubit gate
applyNQbitGateOpDen :: Op -> [Int] -> StQ -> StQ
applyNQbitGateOpDen g nums s
...
-- 2nd one is the target one), when the input state is s. If list nums does not contain only two
-- elements, there is an error.
applyOpDenCPh :: AExp -> [Int] -> StQ -> StQ
...
-- state i n s = state of the system whose initial state is s, after measuring its n-th qubit to be
-- in state |i>, with i=0,1
stateMeasOpDen :: Int -> Int -> StQ -> StQ
...
--- Garbage collection
apply3QbitGate :: Op -> [Int] -> StQ -> StQ
apply3QbitGate g nums s
  | (length nums /= 3) =  error "First argument of function apply3QbitGate must be a list with three elements."
  | otherwise = if ((length $ nub nums) /= (length nums))
                then error "The qubits in apply3QbitGate should be different"
                else mult matrix s
                     where q1 = head nums
                           q_rest = tail nums
                           nqubits = numQubits s
                           ...
                           i_div = (nrows g) `div` 2
                           (m_tl, m_tr, m_bl, m_br) = splitBlocks i_div i_div  g
                           m_A = aux_func m_tl q_rest nqubits listId
                           m_B = aux_func m_tr q_rest nqubits listId
                           m_C = aux_func m_bl q_rest nqubits listId
                           m_D = aux_func m_br q_rest nqubits listId
                           matrix00 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m0 [q1]) m) m_A
                           matrix01 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m01 [q1]) m) m_B
                           matrix10 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m10 [q1]) m) m_C
                           matrix11 = sumListMat $ map (\m -> (tensorProduct . replaceByGate m1 [q1]) m) m_D
...
aux_func :: Op -> [Int] -> Int -> [Op] -> [[Op]]
aux_func g nums nqubits listOp = [matrix00, matrix01, matrix10, matrix11]
  where q1 = head nums
        q2 = last nums
        (m_tl, m_tr, m_bl, m_br) = splitBlocks 2 2 g
        matrix00 = replaceByGate m0 [q1] (replaceByGate m_tl [q2] listOp)
...
check3Qbit :: Op -> Op
check3Qbit op = sumMatrices matrixTop matrixBottom
  where n_rows = nrows op
        i_div = n_rows `div` 2
        (op_tl, op_tr, op_bl, op_br) = splitBlocks i_div i_div op
        m_tl = tensorProduct [m0, op_tl]
        m_tr = tensorProduct [m01, op_tr]
        m_bl = tensorProduct [m10, op_bl]
        m_br = tensorProduct [m1, op_br]
        matrixTop = sumMatrices m_tl m_tr
        matrixBottom = sumMatrices m_bl m_br
        

test_apply3qbitgate :: Op -> [Int] -> [StQ]
test_apply3qbitgate g nums = map (\s -> apply3QbitGate g nums s) states
...
vectToKetComplex :: StQ -> String
vectToKetComplex s = let m = toLists s
                 in toKetComplex (vecToFactors m 0) (vecnumQbits $ length m)

vecnumQbits :: Int -> Int
vecnumQbits n = ceiling $ logBase 2 (fromIntegral n)

vecRmvPlus :: String -> String
vecRmvPlus [] = ""
vecRmvPlus s = let n = length s
            in if s!!(n-2)=='+' then take (n-3) s
               else s

vecToFactors :: [[Complex Double]] -> Int -> [[[Complex Double]]]
vecToFactors [] _ = []
vecToFactors (h:t) i = (vecToFactorsAux h (i,0) (length h)) ++ (vecToFactors t (i+1))

vecToFactorsAux :: [Complex Double] -> (Int,Int) -> Int -> [[[Complex Double]]]
vecToFactorsAux [] _  _ = []
vecToFactorsAux (h:t) (i,j) n = if h == 0 :+ 0
                      then vecToFactorsAux t (i,j+1) n
                      else [ket] : vecToFactorsAux t (i,j+1) n
  where ket = (replicate i (0 :+ 0)) ++ [h] ++ (replicate (n-i-1) (0 :+ 0))

toKetComplex :: [[[Complex Double]]] -> Int -> String
toKetComplex [] _ = []
toKetComplex (h:[]) nqbits = toKetComplexAux h nqbits
toKetComplex (h:t) nqbits = (toKetComplexAux h nqbits) ++ " + " ++ (toKetComplex t nqbits)

toKetComplexAux :: [[Complex Double]] -> Int -> String
toKetComplexAux [] nqbits = error "Unexpected empty list"
toKetComplexAux (k:[]) nqbits = toKet k nqbits
toKetComplexAux (k:t) nqbits = error "Not expecting list with more than two elements"

toKet :: [Complex Double] -> Int -> String
toKet v nqbits = vecRmvPlus $ toKetAux v 0 nqbits

toKetAux :: [Complex Double] -> Int -> Int -> String
toKetAux [] _ _ = ""
toKetAux (h:[]) n nqbits = if h == 0 :+ 0 then ""
                           else "(" ++ veccomplexToString(h) ++ ")|" ++ (vecToBin nqbits n) ++ ">"
toKetAux (h:t) n nqbits = if h == 0 :+ 0 then toKetAux t (n+1) nqbits
                       else  "(" ++ veccomplexToString(h) ++ ")|" ++ (vecToBin nqbits n) ++ "> + " ++ toKetAux t (n+1) nqbits



vecToBin :: Int -> Int -> String
vecToBin nbits n = padZeros nbits (reverse $ vecToBinAux n)
  where
    vecToBinAux 0 = []
    vecToBinAux n = intToDigit (n `mod` 2) : vecToBinAux (n `div` 2)
    padZeros nqbits str = replicate (nbits - length str) '0' ++ str

veccomplexToString :: Complex Double -> String 
veccomplexToString (r:+i)
  | r==0 && i==0 = ""
  | r==0 && i/=0 = show(i) ++ "i"
  | r/=0 && i==0 = show(r)
  | i<0 = show(r) ++ show(i)++ "i"
