...
def set_partner(family: Family, p1: Person, p2: Person) -> None:
    ...
    old_p1_partner = p1.partner
    old_p2_partner = p2.partner

    if old_p1_partner is not None and old_p1_partner is not p2:
        old_p1_partner.partner = None

    if old_p2_partner is not None and old_p2_partner is not p1:
        old_p2_partner.partner = None
...
class FamilyTreeScene(QGraphicsScene):
    ...
    def compute_tree_layout(self) -> dict[str, tuple[float, float]]:
        ...
        People without parents are placed in the first generation. Children are
        placed one generation below their deepest known parent. Partners are kept
        on the same generation when possible. This is intentionally simple; it is
        a first tree-like layout that still allows manual dragging afterwards.
        """
        if self.family is None:
            return {}
        ...
        for _ in range(max(4, len(people) * 2)):
            ...
            for person in people:
                if person.partner is None or get_person_id(person.partner) not in ids:
                    continue

                pid = get_person_id(person)
                qid = get_person_id(person.partner)
        ...
        for level in sorted(people_by_level):
            ...
            # Keep couples adjacent when they are on the same generation.
            for person in row_people:
                ...
                if (
                    person.partner is not None
                    and person.partner in row_people
                    and get_person_id(person.partner) not in placed
                ):
                    ordered_groups.append([person, person.partner])
                    placed.add(pid)
                    placed.add(get_person_id(person.partner))
                else:
                    ordered_groups.append([person])
                    placed.add(pid)
    ...
    def draw_edges(self) -> None:
        ...
        # First create union bullets for explicit partners.
        for person in self.family.fam:
            if person.partner is None or get_person_id(person.partner) not in self.person_items:
                continue

            key = couple_key(person, person.partner)
            if key in drawn_partner_edges:
                continue

            union_item = self.get_or_create_union_item(person, person.partner)
            p1_item = self.get_person_item(person)
            p2_item = self.get_person_item(person.partner)

            if union_item is not None and p1_item is not None and p2_item is not None:
        ...
        for child in self.family.fam:
            ...
            parents_are_partners = (
                father is not None
                and mother is not None
                and father.partner is mother
                and mother.partner is father
            )

            if parents_are_partners:
                key = couple_key(father, mother)
                union_item = self.get_or_create_union_item(father, mother)
                father_item = self.get_person_item(father)
                mother_item = self.get_person_item(mother)

                if (
                    union_item is not None
                    and father_item is not None
                    and mother_item is not None
                    and key not in drawn_partner_edges
                ):
                    self.add_edge_between(father_item, union_item, partner_pen)
                    self.add_edge_between(mother_item, union_item, partner_pen)
                    drawn_partner_edges.add(key)

                if union_item is not None:
                    child_key = (key, get_person_id(child))
...
class DetailsPanel(QFrame):
    ...
    def set_person(self, person: Optional[Person]) -> None:
        ...
        self.info.setText(
            ...
            f"Partner: {person_label(person.partner)}\n"
